...
let cacheFile = path.join(os.tmpdir(), 'resource-uploader-manifest.json');
...
const setConfigFile = (filename) => {
  if (filename) {
    ...
    cacheFile = path.join(os.tmpdir(), `resource-uploader-manifest-${hash}.json`);
  }
};
