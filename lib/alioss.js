...
const upload = (options, isQuiet = false) => {
  ...
  return through2.obj({ maxConcurrency: 8 }, async function(file, enc, cb) {
    ...
    if (options.uriPrefix) {
      ...
      const encodeUri = options.uriPrefix.split('/').map(item => encodeURIComponent(item)).join('/');

      file.cdn = `${options.urlPrefix}!/${encodeUri}/${encodeURIComponent(name)}`;
    }
    else {
