...
const refresh = async (options, url, isQuiet = false) => {
  ...
  if (result.isSuccess) {
    if (isQuiet) {
      console.log(colors.green(encodeUrl(url)));
    }
    else {
...
const upload = (options, isQuiet = false) => {
  ...
  try {
    cdnCache = _(cdnCache).merge(JSON.parse(fs.readFileSync(path.join(os.tmpdir(), 'resource-uploader-manifest.json')))).value();
  }
  catch (e) {
  }
  ...
  return through2.obj({ maxConcurrency: 8 }, async function(file, enc, cb) {
    ...
    fs.writeFileSync(path.join(os.tmpdir(), 'resource-uploader-manifest.json'), JSON.stringify(cdnCache, null, '  '));
